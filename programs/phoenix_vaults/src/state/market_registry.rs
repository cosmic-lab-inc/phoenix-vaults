...
const_assert_eq!(MarketRegistry::SIZE, std::mem::size_of::<MarketRegistry>() + 8);

impl MarketRegistry {
    pub fn deserialize_lookup_table(
        auth: Pubkey,
        lut_data: &[u8]
    ) -> Result<AddressLookupTable> {
        let lookup_table = AddressLookupTable::deserialize(lut_data)
            .map_err(|_| anchor_lang::error::Error::from(
                crate::error::ErrorCode::InvalidAddressLookupTableData
            ))?;
