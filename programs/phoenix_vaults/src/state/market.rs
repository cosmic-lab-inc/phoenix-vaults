...
for Context<'_, '_, 'a, 'info, T>
{
    ...
    fn load_sol_usdc_market(
        ...
        let account = self.remaining_accounts.get(params.sol_usdc_market_index as usize)
        ...
        if header.quote_params.mint_key != params.usdc_mint || header.base_params.mint_key != params.sol_mint {
