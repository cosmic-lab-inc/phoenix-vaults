...
pub fn protocol_liquidate_sol_market<'c: 'info, 'info>(
    ...
    let sol_atoms_to_withdraw = if sol_lots_in_sol_market_terms_to_withdraw > sol_lots_available {
        ...
        let params = ProtocolLiquidateSolMarket::build_swap_params(bl_to_sell)?;
    ...
    // sell SOL into USDC
    let params = ProtocolLiquidateSolMarket::build_swap_params(
        sol_base_lots_on_sol_usdc_market_to_withdraw,
    )?;
...
#[derive(Accounts)]
pub struct ProtocolLiquidateSolMarket<'info> {
    #[account(
        mut,
...
impl<'info> ProtocolLiquidateSolMarket<'info> {
