...
/// Protocol has authority to liquidate vault position in any market if they can't withdraw their equity.
...
pub fn protocol_liquidate_usdc_market<'c: 'info, 'info>(
    ...
    if let Err(e) = vault.check_liquidator(&ctx.accounts.protocol, now) {
    ...
    let amount = shares_to_amount(
        vault.last_protocol_withdraw_request.shares,
        vault.total_shares,
        vault_equity,
    )?;
    let withdraw_request_amount = amount.min(vault.last_protocol_withdraw_request.value);
    ...
    if withdraw_ql > vault_ql {
        ...
        drop(header);
        drop(account_data);
        let params = ProtocolLiquidateUsdcMarket::build_swap_params(bl_to_sell)?;
...
#[derive(Accounts)]
pub struct ProtocolLiquidateUsdcMarket<'info> {
    #[account(
        mut,
        constraint = is_liquidator_for_vault(&vault, &protocol)?,
    ...
    #[account(
        mut,
        constraint = is_usdc_mint(&vault, &protocol_usdc_token_account.mint)?,
        token::mint = usdc_mint,
        token::authority = protocol,
    )]
    pub protocol_usdc_token_account: Account<'info, TokenAccount>,
...
impl<'info> ProtocolLiquidateUsdcMarket<'info> {
...
impl<'info> PhoenixWithdrawCPI for Context<'_, '_, '_, 'info, ProtocolLiquidateUsdcMarket<'info>> {
...
impl<'info> PhoenixTradeCPI for Context<'_, '_, '_, 'info, ProtocolLiquidateUsdcMarket<'info>> {
...
impl<'info> TokenTransferCPI for Context<'_, '_, '_, 'info, ProtocolLiquidateUsdcMarket<'info>> {
